...
def search(
    *,
    ...
    - Lay ngau nhien 1 neighbor trong tap cac neighbor co heuristic tot hon.
    ...
    while expansions < max_expansions:
        ...
        selected, selected_h = random.choice(improving_neighbors)
        ...
        append_trace(
            ...
            formatter=formatter,
            note=(
                f"Random move among {len(improving_neighbors)} improving neighbors: "
                f"{selected.action} (h: {current_h} -> {selected_h})"
            ),
        )
        ...
        if is_goal(current.state):
            append_trace(
                ...
                reached_order=reached_order,
                formatter=formatter,
                note=f"GOAL reached by Stochastic Hill Climbing (h={current_h})",
            )
            path = current.path()
            return SearchResult(
                True,
                ...
                "Goal found by Stochastic Hill Climbing.",
                expansions,
            )
    ...
    return SearchResult(
        ...
        trace,
        trace_by_state,
        f"Stochastic Hill Climbing stopped after expansions={expansions}, depth={current.depth}, h={current_h}.",
        expansions,
    )
